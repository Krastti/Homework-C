...
``` mermaid
flowchart TD
    A([Начало]) --> B[Инициализация min = 10^6, max = -10^6 <br> Объявляем переменную x]
    B --> C[/Ввести x/]
    C --> D{Ввод завершен?}
    D -- Нет --> F{x > max}
    F -- Да --> G[Присвоить max = x]
    F -- Нет --> H{x < min}
    H -- Нет --> I[Ничего не делать]
    H -- Да --> J[Присвоить min = x]
    I --> K[Следующий ввод]
    G --> K
    J --> K
    K --> D
    D -- Да --> L[Вывести max, min]
    L --> M([Конец])
```
### Задание 1.2.25
``` mermaid
...
``` mermaid
flowchart TD
    A([Начало]) --> B[Инициализация min = 10^6, count = 1 <br> Объявление переменной n]
